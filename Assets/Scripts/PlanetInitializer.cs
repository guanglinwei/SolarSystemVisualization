...
public class PlanetInitializer : MonoBehaviour
{
    ...
    private static float planetScale = 10f/6371; // Earth's radius scaled from 6371km to 10 units
    private static float velocityScale = 1e-6f; // Velocity scaled from km/s to units/s
    //private static float massScale = 1e-28f;
    //private float distanceScale = 1e-9f;  // Distance from Earth to Sun scaled from 1.5e11m to 150 units.
    ...
    public void InitPlanets()
    {
        ...
        if (sun == null)
        {
            sun = Instantiate(planetPrefab, Vector3.zero, Quaternion.identity);
        ...
        GravityObject sunGravityObject = sun.GetComponent<GravityObject>();
        if (sunGravityObject == null)
        {
            sunGravityObject = sun.AddComponent<GravityObject>();
        }

        // sunGravityObject.mass = 1989100e6f;
        sunGravityObject.mass = 198.9f;
        sunGravityObject.radius = 695700 * planetScale * 0.07f;
